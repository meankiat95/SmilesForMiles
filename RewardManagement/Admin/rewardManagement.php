...
function addReward( $rewardName, $rewardPoints, $rewardDescription, $rewardContactInfo, $imagetmp) {
    ...
    if (!empty($rewardName) && !empty($rewardPoints) && $rewardPoints > 0 && !empty($rewardDescription) && !empty($imagetmp)) { //Ensure that required details are not null before sent to DB
		...
		$insert = "INSERT INTO reward (rewardID, rewardName, rewardPoints, rewardDescription, rewardContactInfo, rewardImagePath) VALUES ( ?, ?, ?, ?, ?, ?)";
		$stmt = $db->prepare($insert);
		$stmt->bind_param("ssssss", $rewardID, $rewardName, $rewardPoints, $rewardDescription, $rewardContactInfo, $imagetmp);
		}
		
	else{
          echo ("<script LANGUAGE='JavaScript'> window.alert('Fail to add new rewards. Some required fields are missing or incorrect.'); window.location.href='viewRewards.php'; </script>");
        }
...
function modifyReward($rewardID, $rewardName, $rewardPoints, $rewardDescription, $rewardContactInfo, $imagetmp) {
    ...
    if (!empty($rewardID) && is_numeric($rewardID)) { //Ensure rewardID is not empty and is number
        if (!empty($rewardName) && !empty($rewardPoints) && $rewardPoints > 0 && !empty($rewardDescription)) { //Ensure that required details are not null before sent to DB
            ...
            $stmt->execute();
            $stmt->close();
            echo ("<script LANGUAGE='JavaScript'> window.location.href='viewRewards.php';</script>");
        } 
        else {
            echo ("<script LANGUAGE='JavaScript'> window.alert('Reward details failed to update. Some required fields are missing or incorrect.'); window.location.href='viewRewards.php'; </script>");
        }
    } 
    else {
        echo ("<script LANGUAGE='JavaScript'>window.alert('Reward update fail. RewardID is invalid.'); window.location.href='viewRewards.php'; </script>");
    }
}
...
function deactivateReward($rewardID){ //Deactivate reward according to rewardID
	...
	$stmt->execute();
	$db->close();
	echo ("<script LANGUAGE='JavaScript'> window.alert('Reward deactivated successfully.');window.location.href='viewRewards.php';</script>");
...
function activateReward($rewardID){ //Deactivate reward according to rewardID
	...
	$stmt->execute();
	$db->close();
	echo ("<script LANGUAGE='JavaScript'> window.alert('Reward Re-activated successfully.');window.location.href='viewRewards.php';</script>");

}

function listAllActivatedRewards(){ // To retrieve all activated rewards' detail in reward table - Method used by viewRewards.php
	include 'db.php';
...
function listAllDeactivatedRewards(){ // To retrieve all deactivated rewards' detail in reward table - Method used by viewRewards.php
	include 'db.php';
...
function detectNewRewardID($rewardID) { //Check for if this rewardID exist
	include 'db.php';
    if (!empty($rewardID)) {
        $insert = " SELECT * FROM mydatabase.reward WHERE rewardID = '$rewardID'";
        $stmt = $db->prepare($insert);
        $stmt->execute();
        $result = $stmt->get_result();
        if (!empty($result))
